<template>
  ...
  const RATING_API_URL = 'http://localhost:5003';
    ...
    methods: {
      ...
      async fetchUserProfile() {
        ...
        try {
          ...
          const authResponse = await axios.get(`${AUTH_API_URL}/check-auth`, { withCredentials: true });
          ...
          if (authResponse.data.code === 200 && authResponse.data.data.authenticated) {
            ...
            const response = await axios.get(`${AUTH_API_URL}/user/${userId}`, { withCredentials: true });
            ...
            if (response.data.code === 200) {
              ...
              try {
                const phoneResponse = await axios.get(`${AUTH_API_URL}/user/getPhoneFromUser/${userId}`, { withCredentials: true });
              ...
              // Fetch user ratings from the Rating API
              try {
                // Convert user ID to integer if it's a numeric string
                // Note: Adjust the conversion logic based on your actual user ID format
                const numericUserId = parseInt(userId.replace(/\D/g, ''));
                
                if (!isNaN(numericUserId)) {
                  const ratingsResponse = await axios.get(`${RATING_API_URL}/userRating/${numericUserId}`);
                  
                  if (ratingsResponse.data && Array.isArray(ratingsResponse.data)) {
                    // Transform the rating data to match our component's expected format
                    this.userRatings = ratingsResponse.data.map(rating => ({
                      stars: rating.Stars || rating.stars || 0,
                      comment: rating.Comment || rating.comment || "",
                      date: rating.RatedDate || rating.ratedDate || new Date().toISOString(),
                      rater: {
                        name: rating.RaterName || rating.raterName || "Anonymous",
                        avatar: null
                      }
                    }));
                  } else {
                    console.log('No ratings found or unexpected format:', ratingsResponse.data);
                    this.userRatings = [];
                  }
                } else {
                  console.warn('Could not convert user ID to number for ratings API');
                  this.userRatings = [];
                }
              } catch (ratingErr) {
                console.error('Error fetching user ratings:', ratingErr);
                this.userRatings = [];
              }
            } else {
